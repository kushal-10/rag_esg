...
def get_keywords(sdgs_path):
    with open(sdgs_path, "r") as f:
        sdgs_data = json.load(f)
    return list(sdgs_data.values()) + KEYWORDS_EXTRA

def embed_texts(model, sentences, batch_size=BATCH_SIZE):
    return model.encode(sentences, batch_size=batch_size, device=DEVICE, show_progress_bar=False, normalize_embeddings=True)

def assign_scores(json_path, keywords, key_embeds, model):
