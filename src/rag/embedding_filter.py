"""Filter sentences using embedding and fuzzy scores."""
...
def build_embedding_filters(threshold: float = THRESHOLD) -> None:
    """Generate ``embedding_filter.json`` files for all ``sentence_scores.csv`` files.

    Args:
        threshold: Minimum score required for a sentence to be kept.
    """

    # Find all sentence_scores.csv files
    csv_paths: List[str] = glob.glob("data/textsv2/*/*/sentence_scores.csv")
