...
for csv_path in tqdm(csv_files, desc="Processing company/year files"):
    ...
    with open(splits_path, "r") as f:
        splits = json.load(f)
    # If your splits.json keys are ints as strings, convert chunk_id to str
    df['chunk_id'] = df['chunk_id'].astype(str)

    # 2. Get the sentence text for each row
    df['sentence'] = df['chunk_id'].map(splits)

    # 3. Add fuzzy columns for each AI term, with logging
    for term, colname in zip(ai_terms, ai_colnames):
        ...
        for chunk_id, sent in zip(df['chunk_id'], tqdm(df['sentence'], desc=tqdm_desc, leave=False)):
            if sent is not None:
                ...
                match = 1 if score >= fuzzy_threshold else 0
...
print(f"All CSVs updated with fuzzy columns (threshold={fuzzy_threshold}).\nMatches logged to {log_path}")
